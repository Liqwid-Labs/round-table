...
const NewMultisigWallet: FC = () => {
  ...
  return <EditMultisigWallet cardano={cardano} params={params} />;
};
  ...
  return (
    <div className={className}>
      <header>
        ...
        <p>
          Make sure you write down the 24 words of your wallet recover phase{" "}
          <strong className="font-semibold">
            on a piece of paper in the exact order shown here
          </strong>
          .
        </p>
      </header>
      <ul className="grid grid-cols-3 gap-2">
        {words.map((word, index) => (
          <li key={index} className="flex border rounded divide-x">
            <div className="flex-none w-9 text-right px-2 py-1 tabular-nums bg-gray-100">
              {index + 1}
            </div>
      ...
      <footer className="flex justify-end space-x-2">
        {cancel && (
          <button
            className="border rounded text-sky-700 border rounded p-2"
            onClick={cancel}
          >
            Cancel
          </button>
        ...
        <button
          className="border rounded bg-sky-700 text-white p-2"
          onClick={confirm}
        >
...
const WordInput: FC<{
  ...
  const className = useMemo(
    () =>
      [
        ...
        valid ? "text-sky-700" : "text-red-500",
      ].join(" "),
    [valid],
  );
  ...
  return (
    <li className={className} key={index}>
      <div className="flex-none w-9 text-right px-2 py-1 tabular-nums text-black bg-gray-100">
        {index + 1}
      </div>
      <input
...
const RecoverHDWallet: FC<{
  ...
  const wordset: Set<string> = useMemo(
    () => new Set(wordlists[language]),
    [language],
  );
  const length = 24;
  ...
  const setWord = useCallback(
    (index: number, word: string) =>
      setWords(words.map((w, i) => (index === i ? word : w))),
    [words],
  );
  const [BIP32Passphrase, setBIP32Passphrase] = useState<string>("");
  const [repeatBIP32Passphrase, setRepeatBIP32Passphrase] =
    useState<string>("");
  ...
  const isPhraseValid = useMemo(
    () => words.length === length && words.every((word) => wordset.has(word)),
    [words, wordset],
  );
  const buildKey = useCallback(async () => {
    if (!isPhraseValid) throw new Error("Invalid recover phrase");
    if (BIP32Passphrase !== repeatBIP32Passphrase)
      throw new Error("Passphrases do not match");
    const phrase = words.join(" ");
    const entropy = Buffer.from(mnemonicToEntropy(phrase), "hex");
    const key = cardano.lib.Bip32PrivateKey.from_bip39_entropy(
      entropy,
      Buffer.from(BIP32Passphrase, "utf8"),
    );
    return SHA256Digest(key.as_bytes())
      .then(async (buffer) => {
        ...
        if (duplicate)
          throw new Error(`This phrase is a duplicate of ${duplicate.name}`);
      })
      .then(() => setRootKey(key))
  ...
  return (
    <Panel>
      <div className="p-4 space-y-2">
        ...
        <nav>
          <button
            className="py-1 px-2 border rounded text-sm text-sky-700"
            onClick={openModal}
          >
          ...
          {modal === "new" && (
            <Modal
              className="bg-white p-4 rounded w-full md:w-1/2 lg:w-1/3"
      ...
      <footer className="flex justify-end p-4 bg-gray-100">
        <button
          disabled={
            !isPhraseValid ||
            BIP32Passphrase !== repeatBIP32Passphrase ||
            !isPhraseValid
          }
          onClick={buildKey}
  ...
  const create = useCallback(async () => {
    ...
    encryptWithPassword(rootKeyBytes, password, id)
      .then(async (ciphertext) => {
        ...
        const personalIndices = await cardano.generatePersonalAccount(
          wallet,
          password,
          0,
        );
        const multisigIndices = await cardano.generateMultisigAccount(
          wallet,
          password,
          0,
        );
        const keyHashIndices = [personalIndices, multisigIndices].flat();
        createPersonalWallet(wallet, keyHashIndices).then(() =>
    ...
    <Panel>
      <div className="p-4 space-y-2">
        ...
        <div className="space-y-1">
          <div className="after:content-['*'] after:text-red-500">
            Signing Password
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <StrongPasswordInput
...
const NewPersonalWallet: FC = () => {
  ...
  if (!rootKey)
    return <RecoverHDWallet cardano={cardano} setRootKey={setRootKey} />;

  return <SavePersonalWallet cardano={cardano} id={id} rootKey={rootKey} />;
};
  ...
  return (
    <Layout>
      <Hero>
        ...
        <div>
          <p>
            Multisig wallet is made of multiple signers or time ranges of
            validity.
          </p>
          <p>
            Personal wallet is for normal use like other wallets, e.g.
            Nami/Eternal/etc. It also holds addresses for multisig wallet
            making.
          </p>
        </div>
        <div className="flex">
