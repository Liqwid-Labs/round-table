...
const Spend: FC<{
  ...
  const buildInputResult = useCallback(
    (builder: SingleInputBuilder) => {
      if (typeof policy === "string") return builder.payment_key();
      return builder.native_script(
        cardano.getPaymentNativeScriptFromPolicy(policy),
        cardano.lib.NativeScriptWitnessInfo.assume_signature_count(),
      );
    },
    [cardano, policy],
  );
  const buildCertResult = useCallback(
    (builder: SingleCertificateBuilder) => {
      if (typeof policy === "string") return builder.payment_key();
      return builder.native_script(
        cardano.getStakingNativeScriptFromPolicy(policy),
        cardano.lib.NativeScriptWitnessInfo.assume_signature_count(),
      );
    },
    [cardano, policy],
  );
  const buildWithdrawalResult = useCallback(
  ...
  if (loading || !data) return <PartialLoading />;
...
const ShowNativeScript: FC<{
  ...
  if (typeof policy === "string")
    throw new Error("No NativeScript for policy in single address");
...
const GetPolicy: NextPage = () => {
  ...
  const result:
    | { policy: Policy; address: string; rewardAddress: string }
    | undefined = useMemo(() => {
    if (!cardano || !policyContent) return;
    if (typeof policyContent !== "string")
      throw new Error("Cannot parse the policy");
    const { Address } = cardano.lib;
    if (Address.is_valid_bech32(policyContent))
      return {
        policy: policyContent,
        address: policyContent,
        rewardAddress: cardano
          .getPolicyRewardAddress(policyContent, isMainnet(config))
          .to_address()
          .to_bech32(),
      };
    const policy: Policy = JSON.parse(policyContent);
    const address = cardano
      .getPolicyAddress(policy, isMainnet(config))
      .to_bech32();
    const rewardAddress = cardano
      .getPolicyRewardAddress(policy, isMainnet(config))
      .to_address()
      .to_bech32();
    return { policy, address, rewardAddress };
  }, [cardano, config, policyContent]);
  const [tab, setTab] = useState<
    "summary" | "spend" | "edit" | "remove" | "native script"
  >("summary");
  const multisigWallet = useLiveQuery(
    async () => result && db.multisigWallets.get(result.address),
    [result],
  );
      ...
      {cardano && result && (
        <div className="space-y-2">
          <Hero>
            ...
            <div className="text-sm">
              {multisigWallet && multisigWallet.description.length > 0 && (
                <div>{multisigWallet.description}</div>
              )}
            </div>
            <div className="flex">
            ...
            {tab === "edit" && (
              <div className="flex items-center space-x-1">
                ...
                <span>
                  You can create a new account by editing the policy. The assets
                  in the original one will remain untouched.
                </span>
              </div>
            )}
          </Hero>
          ...
          {tab === "edit" && walletParams && (
            <EditMultisigWallet cardano={cardano} params={walletParams} />
          )}
          {tab === "remove" && multisigWallet && (
            <RemoveWallet
              walletName={multisigWallet.name}
              remove={removeWallet}
            />
          )}
          {tab === "native script" && typeof result.policy !== "string" && (
            <ShowNativeScript cardano={cardano} policy={result.policy} />
          )}
        </div>
      )}
    </Layout>
