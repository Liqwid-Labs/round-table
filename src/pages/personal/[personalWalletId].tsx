...
import {
  ...
  updatePersonalWalletAndDeindex,
} from "../../db";
...
const Multisig: FC<{
  ...
  const account = useMemo(
    () => wallet.multisigAccounts.get(accountIndex),
    [wallet.multisigAccounts, accountIndex],
  );
  const addresses = useMemo(
    () =>
      account &&
      cardano?.getAddressesFromMultisigAccount(account, isMainnet(config)),
    [cardano, account, config],
  );

  if (!cardano || !addresses)
    return (
      <Modal>
        <Loading />
      </Modal>
  ...
  return (
    <Panel>
      ...
      <footer className="flex justify-end p-4 bg-gray-100">
        <button
          onClick={addAddress}
          className="flex space-x-1 px-4 py-2 bg-sky-700 text-white rounded"
        >
          Add Address
        </button>
      </footer>
...
const Personal: FC<{
  ...
  const account = useMemo(
    () => wallet.personalAccounts.get(accountIndex),
    [wallet.personalAccounts, accountIndex],
  );
  const addresses = useMemo(
    () =>
      account &&
  ...
  const rewardAddress = useMemo(
    () =>
      account &&
      cardano
        ?.readRewardAddressFromPublicKey(account.publicKey, isMainnet(config))
        .to_address()
        .to_bech32(),
    [cardano, config, account],
  );
  const [tab, setTab] = useState<"summary" | "receive" | "spend">("summary");
  const selectAccount: ChangeEventHandler<HTMLSelectElement> = useCallback(
    (e) => setAccountIndex(parseInt(e.target.value)),
    [],
  );
  ...
  const addAccount = useCallback(
    async (password: string) => {
      ...
      cardano
        .generatePersonalAccount(wallet, password, newAccountIndex)
        .then((indices) => {
  ...
  const deleteAccount = useCallback(() => {
    ...
    updatePersonalWalletAndDeindex(wallet, keyHashes).then(() =>
      setAccountIndex(0),
    );
  }, [account, accountIndex, wallet]);

  if (!addresses || !rewardAddress || !cardano)
    return (
      <Modal>
        <Loading />
      </Modal>
  ...
  return (
    <div className={className}>
        ...
        <div className="flex space-x-2">
          ...
          <nav className="text-sm rounded border-white border divide-x overflow-hidden">
            <select
              ...
              className="px-2 py-1 bg-sky-700"
            >
              {Array.from(wallet.personalAccounts, ([index, _]) => (
      ...
      {tab === "summary" && (
        <Summary addresses={addresses} rewardAddress={rewardAddress}>
          <footer className="flex justify-end p-4 bg-gray-100">
            <ConfirmModalButton
...
const ShowPersonalWallet: NextPage = () => {
  ...
  const [tab, setTab] = useState<"personal" | "multisig" | "edit" | "remove">(
    "personal",
  );
  const { notify } = useContext(NotificationContext);
  ...
  return (
    <Layout>
      <Hero>
        ...
        {tab === "personal" && (
          <div className="flex" id="personal-subtab"></div>
        )}
      </Hero>
      {tab === "personal" && (
        <Personal wallet={personalWallet} className="space-y-2" />
      )}
      {tab === "multisig" && (
        <>
          <div className="p-4 text-yellow-700 bg-yellow-100 rounded shadow flex items-center space-x-1">
            ...
            <strong className="font-semibold">
              DO NOT USE THEM TO RECEIVE FUNDS.
            </strong>
          </div>
      ...
      {tab === "remove" && (
        <RemoveWallet walletName={personalWallet.name} remove={removeWallet} />
      )}
    </Layout>
  );
};
