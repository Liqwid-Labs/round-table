...
type PersonalWallet = BasicInfoParams &
  Timestamp & {
    id: number;
    hash: Uint8Array;
    rootKey: Uint8Array;
...
const createPersonalWallet = (
  ...
  indices: KeyHashIndex[],
) =>
  db.transaction("rw", db.personalWallets, db.keyHashIndices, async () => {
    return db.personalWallets
      .add(wallet)
      .then(() => db.keyHashIndices.bulkPut(indices));
  });
const updatePersonalWallet = (
  wallet: PersonalWallet,
  indices: KeyHashIndex[],
) =>
  db.transaction("rw", db.personalWallets, db.keyHashIndices, async () => {
...
const deletePersonalWallet = (wallet: PersonalWallet) =>
  db.transaction("rw", db.personalWallets, db.keyHashIndices, async () => {
    const walletId = wallet.id;
    return db.personalWallets
      .delete(walletId)
      .then(() => db.keyHashIndices.where({ walletId }).delete());
  });
const updatePersonalWalletAndDeindex = (
  wallet: PersonalWallet,
  keyHashes: Uint8Array[],
) =>
  db.transaction("rw", db.personalWallets, db.keyHashIndices, async () => {
    return db.personalWallets
      .put(wallet)
