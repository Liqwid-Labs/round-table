...
test("UserData", () => {
        ...
        multisigAccounts: new Map([
          [
            0,
            {
              ...
              addresses: [
                {
                  paymentKeyHash: new Uint8Array([6]),
                  stakingKeyHash: new Uint8Array([7]),
                },
              ],
            },
          ],
  ...
  expect(multisigWallet.id).toBe(
    "addr_test1xpff750879d8u3wvl9yhedaff0gv8zmecxzae3h9ty9wrtz9uf28cs7rk7m67rmse675wltjhya740893pwzgkvqqh8q67usf2",
  );
  expect(multisigWallet.name).toBe("MWallet");
  expect(multisigWallet.description).toBe("xxx");
  if (typeof multisigWallet.policy === "string")
    throw new Error("Wrong policy");
