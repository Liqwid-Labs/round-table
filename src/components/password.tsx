...
const PasswordInput: FC<{
  ...
  const inputType: HTMLInputTypeAttribute = useMemo(
  ...
  return (
    <label
      className={[
        "flex w-full border rounded items center ring-sky-500 focus-within:ring-1",
        className,
      ].join(" ")}
    >
      <div className="p-2">
...
const testPasswordLength: PasswordTester = (password) =>
  password.length >= MIN_LENGTH;
const testPasswordUpperCase: PasswordTester = (password) =>
  (password.match(/[A-Z]/g)?.length ?? 0) >= MIN_UPPERCASE;
const testPasswordLowerCase: PasswordTester = (password) =>
  (password.match(/[a-z]/g)?.length ?? 0) >= MIN_LOWERCASE;
const testPasswordDigits: PasswordTester = (password) =>
  (password.match(/\d/g)?.length ?? 0) >= MIN_DIGITS;
const testPasswordSpecials: PasswordTester = (password) =>
  (password.match(/\W/g)?.length ?? 0) >= MIN_SPECIALS;
...
const isPasswordStrong: PasswordTester = (password) =>
  testPasswordSuits.every((fn) => fn(password));
...
const AskPasswordModalButton: FC<{
  ...
  return (
    <>
      <button onClick={() => setModal(true)} className={className}>
        {children}
      </button>
      {modal && (
        <Modal
        ...
        >
          <PasswordBox disabled={disabled} title={title} onConfirm={confirm}>
            Confirm
          </PasswordBox>
        </Modal>
      )}
