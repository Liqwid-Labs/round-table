...
const DerivationPath: FC<{
  ...
  const keyHashIndex = useLiveQuery(
    async () => keyHash && db.keyHashIndices.get(keyHash),
    [keyHash],
  );
  const derivationPath = useMemo(
    () => keyHashIndex?.derivationPath,
    [keyHashIndex],
  );

  if (!derivationPath) return null;

  return <>{formatDerivationPath(derivationPath)}</>;
};
...
const SlotInput: FC<{
  ...
  const [slot, setSlot] = useState<number>(
    initialSlot ?? estimateSlotByDate(new Date(), config.network),
  );
  const date = estimateDateBySlot(slot, config.network);
  const changeDate = useCallback(
    (date: Date) => setSlot(estimateSlotByDate(date, config.network)),
    [config.network],
  );
  ...
  return (
    <div className={className}>
      <NumberInput
        className="block w-full p-2 border rounded ring-sky-500 focus:ring-1"
        min={0}
        step={1000}
        value={slot}
        onCommit={setSlot}
      ...
      <nav className="flex justify-end space-x-2">
        <button onClick={cancel} className="border rounded p-2 text-sky-700">
          Cancel
        </button>
        <button
...
const AddTimelock: FC<{
  ...
  const [type, setType] = useState<"TimelockStart" | "TimelockExpiry">(
    "TimelockStart",
  );
  const now = useLiveDate();
  const confirmSlot = useCallback(
    (slot: number) => add({ type, slot }),
    [add, type],
  );
    ...
    <div className={className}>
      ...
      {type === "TimelockStart" && (
        <div className="p-2 rounded bg-red-700 text-white">
          ...
          <p>
            Be careful with this option. It returns false before the slot/time
            is reached. You might not want to wait too long to unlock it.
          </p>
        </div>
      )}
      {type === "TimelockExpiry" && (
        <div className="p-2 rounded bg-red-700 text-white">
          ...
          <p>
            Be careful with this option. It returns false after the slot/time is
            passed which could lead to coins locked/burned forever when it is
            combined with All policy and At Least policy or used alone.
          </p>
        </div>
      )}
      <SlotInput
...
const AddressButton: FC<{
  ...
  const payment = useMemo(
    () =>
      cardano?.parseAddress(address).payment_cred()?.to_keyhash()?.to_bytes(),
    [cardano, address],
  );
  const staking = useMemo(
    () =>
      cardano?.parseAddress(address).staking_cred()?.to_keyhash()?.to_bytes(),
    [cardano, address],
  );
  ...
  return (
    <button onClick={click} className={className}>
      ...
      <div className="flex items-center justify-between text-sm text-sky-700">
        <div>
          <DerivationPath keyHash={payment} />
        </div>
        <div>
          <DerivationPath keyHash={staking} />
        </div>
      </div>
    </button>
  );
...
const AddressButtonGroup: FC<{
  ...
  const totalPage = useMemo(
    () => Math.ceil(addresses.length / perPage),
    [addresses, perPage],
  );
  ...
  return (
    <div className={className}>
      {showingAddresses.map((address) => (
        <AddressButton
          key={address}
          className="p-2 space-y-1 hover:bg-sky-100"
          onClick={onClick}
          address={address}
        />
      ))}
...
const AddAddress: FC<{
  ...
  const signingAddresses: string[] | undefined = useMemo(() => {
    if (signingWallet && signingWallet !== "import") {
      return Array.from(signingWallet.multisigAccounts, ([_, account]) =>
        cardano.getAddressesFromMultisigAccount(account, isMainnet(config)),
      ).flat();
    }
  ...
  return (
    <div className={className}>
      {!signingWallet && (
        <>
          <h2 className="font-semibold p-4 bg-gray-100 text-center">
            Add Signer
          </h2>
          ...
          {signingAddresses && (
            <>
              {signingWallet !== "import" && (
                <h2 className="p-2 bg-gray-100 text-center">
                  {signingWallet.name}
                </h2>
              )}
              <AddressButtonGroup
...
const EditTimelockStart: FC<{
  ...
  return <TimelockStartViewer slot={slot} txStartSlot={currentSlot} />;
};
...
const EditTimelockExpiry: FC<{
  ...
  return <TimelockExpiryViewer slot={slot} txExpirySlot={currentSlot} />;
};
...
const EditPolicy: FC<{
  ...
  const setPolicyType: ChangeEventHandler<HTMLSelectElement> = useCallback(
    (event) => {
      if (!policies) return;
    ...
    <div className={className}>
      <nav className="flex space-x-2 text-sm">
        {policies && policies.length > 0 && (
          <div className="flex border rounded divide-x items-center ring-sky-500 focus-within:ring-1">
            <select
              className="bg-white text-sky-700 px-2 py-1"
              value={policy.type}
        ...
        <ul className={ulClassName}>
          {policies.map((subPolicy, index) => {
            ...
            const setSubPolicy = (subPolicy: Policy) => {
              setPolicy({
                ...policy,
                policies: policies.map((p, i) => (i === index ? subPolicy : p)),
              });
            };
            return (
      ...
      {modal === "address" && (
        <Modal className="w-80" onBackgroundClick={closeModal}>
          <AddAddress
            className="bg-white rounded divide-y overflow-hidden"
            cardano={cardano}
            add={addPolicy}
            cancel={closeModal}
          />
      ...
      <div className="p-4 space-y-2">
        ...
        <div>
          <p>
            ...
            <strong className="font-semibold">{walletName}</strong>?
          </p>
          <p>
            By removing the wallet you will just remove the record in your
            browser. The assets in it remain untouched. Type the wallet name
            below to confirm.
          </p>
        </div>
        <input
...
const Summary: FC<{
  ...
  const result:
    | { balance: Value; reward: bigint; delegation?: Delegation }
    | undefined = useMemo(() => {
    if (!data) return;
    const {
      paymentAddresses,
    ...
    return {
      ...
      delegation: getCurrentDelegation(
        stakeRegistrations_aggregate,
        stakeDeregistrations_aggregate,
        delegations,
      ),
  ...
  if (!result) return <PartialLoading />;
              ...
              {Array.from(balance.assets, ([id, quantity]) => (
                <li key={id} className="p-2 border rounded">
                  <AssetAmount
                    quantity={quantity}
                    decimals={0}
                    symbol={Buffer.from(getAssetName(id), "hex").toString(
                      "ascii",
                    )}
                  />
