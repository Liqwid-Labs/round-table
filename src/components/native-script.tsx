...
const Badge: FC<{
  ...
  return <div className={[baseClassName, className].join(" ")}>{children}</div>;
};
...
const TimelockViewer: FC<{
  ...
  return (
    ...
    >
      <span>{slot}</span>
      <span>
        (est. {estimateDateBySlot(slot, config.network).toLocaleString()})
      </span>
...
const TimelockStartViewer: FC<{
  ...
  return <TimelockViewer slot={slot} isValid={isValid} />;
};
  ...
  return (
    <div className={[className, color].join(" ")}>
      ...
      <nav className="flex space-x-1">
        ...
        {signature && (
          <CopyButton
            copied={<ClipboardDocumentCheckIcon className="w-4" />}
            ms={1000}
            content={signature}
          >
    ...
    return (
      <div className="flex items-center space-x-1">
        ...
        <TimelockExpiryViewer
          slot={slot}
          txExpirySlot={verifyingData?.txExpirySlot}
        />
      </div>
    );
  }
  ...
  if (script) {
    ...
    return (
      <div className="flex items-center space-x-1">
        ...
        <TimelockStartViewer
          slot={slot}
          txStartSlot={verifyingData?.txStartSlot}
        />
      </div>
    );
  }
    ...
    return (
      <div className={className}>
        ...
        <ul className={ulClassName}>
          {Array.from(toIter(script.native_scripts())).map(
            (nativeScript, index) => (
