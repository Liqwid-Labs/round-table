...
const CertificateListing: FC<{
  ...
  const networkId: number = useMemo(
    () => cardano.getNetworkId(isMainnet(config)),
    [cardano, config],
  );
  ...
  if (cert) {
    ...
    const rewardAddress = RewardAddress.new(networkId, cert.stake_credential())
      .to_address()
      .to_bech32();
    return (
      <>
    ...
    <ul className={ulClassName}>
      {certificates.map((certificate, index) => (
        <li className={liClassName} key={index}>
          <CertificateListing cardano={cardano} certificate={certificate} />
        </li>
      ))}
    </ul>
  );
...
const getTxIndex = (input: TransactionInput) =>
  parseInt(input.index().to_str());
...
const TransactionInputViewer: FC<{
  ...
  const recipient = useMemo(
    () => registry?.get(hash)?.get(index),
    [hash, index, registry],
  );

  if (recipient)
    return <RecipientViewer className={className} recipient={recipient} />;
...
const getCIP30Wallet = (name: CIP30WalletName): CIP30Wallet | undefined => {
  ...
  switch (name) {
    ...
    case "nami":
      return cardano?.nami;
    case "gero":
      return cardano?.gerowallet;
    case "flint":
      return cardano?.flint;
    case "typhon":
      return cardano?.typhoncip30;
  }
};
...
const CIP30Names: CIP30WalletName[] = [
  "nami",
  "gero",
  "eternl",
  "flint",
...
const SignTxButtonClassName =
  "flex w-full items-center justify-between py-2 px-4 text-sky-700 disabled:bg-gray-100 disabled:text-gray-500 hover:bg-sky-100";
const SignTxButton: FC<{
  ...
  onSuccess,
  transaction,
  requiredKeyHashHexes,
  children,
}) => {
  const cardano = useCardanoMultiplatformLib();
  const { notify } = useContext(NotificationContext);
  const txHash = useMemo(
    () => cardano?.lib.hash_transaction(transaction.body()).to_bytes(),
    [cardano, transaction],
  );
  ...
  const [signingWallet, setSigningWallet] = useState<
    PersonalWallet | "import" | undefined
  >();
  useEffect(() => {
    if (!modal) setSigningWallet(undefined);
  }, [modal]);
  const signWithPersonalWallet = useCallback(
    async (password: string) => {
      if (!signingWallet || signingWallet === "import" || !cardano || !txHash)
        return;
      cardano
        .signWithPersonalWallet(
          requiredKeyHashHexes,
          txHash,
          signingWallet,
          password,
  ...
  const isDisabled: boolean = useMemo(
    () => !signingWallet || !cardano || !txHash,
    [signingWallet, cardano, txHash],
  );
  const [CIP30Wallets, setCIP30Wallets] = useState(
    new Map<CIP30WalletName, CIP30Wallet>(),
  );
  useEffect(() => {
    setCIP30Wallets(
      CIP30Names.reduce((result, name) => {
  ...
  return (
    <>
      ...
      {modal && (
        <Modal
          className="bg-white divide-y text-center rounded w-80 overflow-hidden"
          onBackgroundClick={closeModal}
        >
          {!signingWallet && (
            <>
              <header>
                <h2 className="font-semibold p-4 bg-gray-100">
                  Choose a wallet
                </h2>
              </header>
              <nav className="divide-y text-sky-700">
                ...
                <button
                  onClick={() => setSigningWallet("import")}
                  className={SignTxButtonClassName}
                >
                  <ArrowUpTrayIcon className="w-6" />
                  <span>Import</span>
                </button>
                {Array.from(CIP30Wallets, ([name, wallet]) => (
                  <CIP30SignTxButton
                    key={name}
                    transaction={transaction}
                    partialSign={true}
                ...
                <button
                  onClick={closeModal}
                  className="w-full text-center text-sky-700 p-2 hover:bg-sky-100"
                >
                  Cancel
                </button>
              </nav>
  ...
  const onClick: MouseEventHandler<HTMLButtonElement> = useCallback(() => {
    wallet
      ...
      .then(async (walletAPI) => {
        ...
        return walletAPI.signTx(toHex(transaction), partialSign).then(sign);
      })
...
const submitTx = (URL: string, body: Uint8Array) =>
  fetch(URL, {
    method: "POST",
...
const SubmitTxButton: FC<{
  ...
  const clickHandle: MouseEventHandler<HTMLButtonElement> = () => {
    ...
    const promises = config.submitAPI.map((URL) =>
      submitTx(URL, transaction.to_bytes()),
    );
    Promise.any(promises)
      ...
      .catch((reason: AggregateError) => {
        const duplicatedSpentError: Error = reason.errors.find(
          (error) => error.name === "DuplicatedSpentError",
        );
        ...
        const invalidSignatureError: Error = reason.errors.find(
          (error) => error.name === "InvalidSignatureError",
        );
    ...
    <li className={className}>
      ...
      <div>
        ...
        <div className="text-sm text-gray-700">
          {wallet?.apiVersion ?? "Not Installed"}
        </div>
      </div>
    </li>
  );
...
const useAutoSync = (
  ...
  useEffect(() => {
    (async () => {
      const cachedSignatures = await fetch(
        "/api/signatureCache/" + toHex(txHash),
      ).then((res) => res.json());
    ...
    (async () => {
      ...
      await Array.from(signers, async (pkh) => {
        ...
        const res = await fetch(
          "/api/signatureCache/" + toHex(txHash) + "/" + pkh,
          {
            method: "POST",
...
const TransactionLoader: FC<{
  ...
  const transaction = useMemo(
    () => cardano?.lib.Transaction.from_bytes(content),
    [cardano, content],
  );

  if (!cardano || !transaction)
    return (
      <Modal>
        <Loading />
      </Modal>
  ...
  return <TransactionViewer cardano={cardano} transaction={transaction} />;
};

type SignatureMap = Map<string, Vkeywitness>;

const updateSignatureMap = (
...
const getRecipientsFromCMLTransactionOutputs = (
  outputs: TransactionOutputs,
): Recipient[] =>
  Array.from(toIter(outputs), (output, index) => {
    ...
    if (multiAsset) {
      Array.from(toIter(multiAsset.keys()), (policyId) => {
        ...
        _asset &&
          Array.from(toIter(_asset.keys()), (assetName) => {
...
const TransactionViewer: FC<{
  ...
  const txHash = useMemo(
    () => cardano.lib.hash_transaction(txBody),
    [cardano, txBody],
  );
  const txWithdrawals = useMemo(() => {
    ...
    Array.from(toIter(withdrawals.keys()), (address) => {
      ...
      if (amount)
        result.set(address.to_address().to_bech32(), BigInt(amount.to_str()));
    });
    return result;
  }, [txBody]);
  ...
  const [requiredPaymentKeys, setRequiredPaymentKeys] = useState<
    Set<string> | undefined
  >();
  const signerRegistry = useMemo(() => {
    const signers = new Set<string>();
    nativeScripts?.forEach((script) => {
      Array.from(toIter(script.get_required_signers()), (signer) =>
        signers.add(toHex(signer)),
      );
    });
  ...
  const { data } = useTransactionSummaryQuery({
    variables: {
      hashes: txInputs.map((input) => input.transaction_id().to_hex()),
    },
  });
  const txInputsRegistry = useMemo(
    () => data && collectTransactionOutputs(data.transactions),
    [data],
  );
  ...
  useEffect(() => {
    ...
    txInputs.forEach((input) => {
      ...
      const keyHash =
        address &&
        cardano.parseAddress(address).payment_cred()?.to_keyhash()?.to_hex();
    ...
    txInputsRegistry,
    txRequiredSigners,
    setRequiredPaymentKeys,
  ]);
  const txOutputs: Recipient[] = useMemo(
    () => getRecipientsFromCMLTransactionOutputs(txBody.outputs()),
    [txBody],
  );
  ...
  return (
    <div className="space-y-2">
      <Hero>
        ...
        <p>
          If the transaction is correct, you can sign and submit it. If this
          transaction needs more than one signers, you can share them this URL
          to get it signed and share the signatures.
        </p>
        <nav>
          <ShareCurrentURLButton className="flex space-x-1 bg-white text-sky-700 py-1 px-2 rounded shadow w-32 justify-center items-center">
      ...
      <Panel>
        <div className="p-4 space-y-2">
                ...
                {txInputs.map((input, index) => (
                  <li key={index} className="p-2 border rounded">
                    <TransactionInputViewer
                      className="space-y-1"
                      input={input}
                      registry={txInputsRegistry}
                    />
                  </li>
                ))}
                {Array.from(txWithdrawals, ([address, amount], index) => (
                  <li key={index} className="p-2 border rounded space-y-1">
                    <AddressableContent content={address} scanType="stakekey" />
                    <div>
                      <ADAAmount lovelace={amount} />
                    </div>
                  </li>
                ))}
              </ul>
          ...
          {txMessage && (
            <div className="space-y-1">
              ...
              <div className="p-2 border rounded text-sm break-all">
                {txMessage.map((line, index) => (
                  <p key={index}>{line}</p>
                ))}
              </div>
            </div>
          )}
          {requiredPaymentKeys && requiredPaymentKeys.size > 0 && (
            <div className="space-y-1">
              <h2 className="font-semibold">Required Payment Signatures</h2>
              <ul className="space-y-1 rounded border p-2 text-sm">
                {Array.from(requiredPaymentKeys, (keyHashHex, index) => (
                  <li key={index}>
                    <SignatureViewer
                      className="flex space-x-1 items-center"
                      signedClassName="text-green-500"
                      name={keyHashHex}
                      signature={cardano.buildSignatureSetHex(
                        signatureMap.get(keyHashHex),
                      )}
                    />
                  </li>
                ))}
          ...
          {requiredStakingKeys && requiredStakingKeys.size > 0 && (
            <div className="space-y-1">
              <h2 className="font-semibold">Required Staking Signatures</h2>
              <ul className="space-y-1 rounded border p-2 text-sm">
                {Array.from(requiredStakingKeys, (keyHashHex, index) => (
                  <li key={index}>
                    <SignatureViewer
                      className="flex space-x-1 items-center"
                      signedClassName="text-green-500"
          ...
          {nativeScripts && nativeScripts.length > 0 && (
            <div className="space-y-1">
...
const AddAssetButton: FC<{
  ...
  const assets = useMemo(
    () =>
      Array.from(budget.assets).filter(
        ([id, quantity]) => !value.assets.has(id) && quantity > BigInt(0),
      ),
    [budget, value],
  );
...
const RecipientAddressInput: FC<{
  ...
  const isValid =
    cardano.isValidAddress(address) &&
    isAddressNetworkCorrect(config, cardano.parseAddress(address));
  ...
  return (
    <div className={className}>
      <label className="flex block border rounded overflow-hidden ring-sky-500 focus-within:ring-1">
        ...
        <input
          className={[
            "p-2 block w-full disabled:bg-gray-100",
      ...
      {address && !isValid && (
        <p className="text-sm text-red-500">The address is invalid.</p>
      )}
    </div>
  );
};
...
const RecipientValueInput: FC<{
  ...
  const selectAsset = useCallback(
    (id: string) => setAsset(id, BigInt(0)),
    [setAsset],
  );
...
const TransactionRecipient: FC<{
  ...
  const minLovelace = useMemo(
    () =>
      cardano.isValidAddress(recipient.address)
        ? getMinLovelace(recipient)
        : undefined,
    [recipient, cardano, getMinLovelace],
  );
...
const TransactionMessageInput: FC<{
  ...
  const isValid = messageLines.every(
    (line) => new TextEncoder().encode(line).length <= 64,
  );
...
const NewTransaction: FC<{
  ...
  buildCertResult: (
    builder: SingleCertificateBuilder,
  ) => CertificateBuilderResult;
  buildWithdrawalResult: (
    builder: SingleWithdrawalBuilder,
  ) => WithdrawalBuilderResult;
  ...
  const [expirySlot, setExpirySlot] = useState<number | undefined>(
    currentSlot + 24 * 60 * 60,
  );
  ...
  const [changeAddress, setChangeAddress] =
    useState<string>(defaultChangeAddress);
  ...
  const [modal, setModal] = useState<
    "delegation" | "start" | "expiry" | undefined
  >();
  const [delegation, setDelegation] = useState<StakePool | undefined>();
  const deposit: bigint = useMemo(() => {
    if (!isRegistered && delegation)
      return BigInt(protocolParameters.keyDeposit);
    return BigInt(0);
  }, [isRegistered, delegation, protocolParameters]);
  const [config, _] = useContext(ConfigContext);
  const donatingAddress = useMemo(
    () => donationAddress(config.network),
    [config.network],
  );
  const [donatingValue, setDonatingValue] = useState<Value | undefined>();
  const donatingRecipient: Recipient | undefined = useMemo(
    () =>
      donatingValue && {
  ...
  const allRecipients = useMemo(
    () => recipients.concat(donatingRecipient ?? []),
    [donatingRecipient, recipients],
  );
  const budget: Value = useMemo(
    () =>
      allRecipients
        .map(({ value }) => value)
  ...
  const stakeRegistration = useMemo(() => {
    if (!isRegistered && delegation)
      return cardano.createRegistrationCertificate(rewardAddress);
  }, [cardano, delegation, rewardAddress, isRegistered]);
  const stakeDelegation = useMemo(() => {
    if (delegation)
      return cardano.createDelegationCertificate(rewardAddress, delegation.id);
  }, [cardano, delegation, rewardAddress]);
  const [stakeDeregistration, setStakeDeregistration] = useState<
  ...
  const auxiliaryData = useMemo(() => {
    if (message.length > 0) {
      ...
      data.add_json_metadatum_with_schema(
        cardano.getMessageLabel(),
        value,
  ...
  const confirmStartSlot = useCallback(
    (slot: number) => {
      setStartSlot(slot);
      closeModal();
    },
  ...
  const confirmExpirySlot = useCallback(
    (slot: number) => {
      setExpirySlot(slot);
      closeModal();
    },
  ...
  useEffect(() => {
    ...
    init().then(() => {
      ...
      const result = select(inputs, outputs, {
        lovelace: minLovelaceForChange,
        assets: [],
      });
      const txOutputs: TransactionOutput[] | undefined = result?.selected.map(
        (output) => output.data,
      );
  ...
  const getMinLovelace = useCallback(
    (recipient: Recipient): bigint =>
      cardano.getMinLovelace(recipient, protocolParameters),
  ...
  const txResult = useMemo(
    () =>
      getResult(() => {
        ...
        const { BigNum, ChangeSelectionAlgo, SingleCertificateBuilder } =
          cardano.lib;
        ...
        if (stakeRegistration)
          txBuilder.add_cert(
            buildCertResult(SingleCertificateBuilder.new(stakeRegistration)),
          );
        if (stakeDeregistration)
          txBuilder.add_cert(
            buildCertResult(SingleCertificateBuilder.new(stakeDeregistration)),
          );
        if (stakeDelegation)
          txBuilder.add_cert(
            buildCertResult(SingleCertificateBuilder.new(stakeDelegation)),
          );

        if (withdrawAll) {
          const builder = cardano.createWithdrawalBuilder(
            rewardAddress,
            availableReward,
          );
        ...
        if (startSlot)
          txBuilder.set_validity_start_interval(
            BigNum.from_str(startSlot.toString()),
          );
        if (expirySlot)
          txBuilder.set_ttl(BigNum.from_str(expirySlot.toString()));

        return txBuilder
          .build(
            ChangeSelectionAlgo.Default,
            cardano.parseAddress(changeAddress),
          )
          .build_unchecked();
      }),
    [
  ...
  const changeRecipient = useCallback(
    (index: number, recipient: Recipient) => {
      setRecipients(
        recipients.map((_recipient, _index) =>
          index === _index ? recipient : _recipient,
        ),
      );
    },
    [recipients],
    ...
    <Panel>
      ...
      {donatingValue && (
        <div>
          ...
          <div className="p-4 space-y-2">
            <div className="space-y-1">
              <div>
                We kindly suggest considering tipping the developer as it would
                greatly contribute to the development and quality of the
                project. Your support is highly appreciated. Thank you for your
                consideration.
              </div>
            </div>
            ...
            <RecipientValueInput
              ...
              budget={budget}
              minLovelace={
                donatingRecipient && getMinLovelace(donatingRecipient)
              }
            />
          </div>
        </div>
      ...
      {delegation && (
        <div>
          ...
          <div className="p-4 space-y-2">
            ...
            {deposit > BigInt(0) && (
              <p className="text-sm">
                This address was not registered for staking. Will deposit{" "}
                <ADAAmount className="font-semibold" lovelace={deposit} /> to
                register.
              </p>
            )}
          </div>
      ...
      <div>
        ...
        <div className="p-4 space-y-2">
          <div className="flex items-center space-x-2">
            <span>Start slot:</span>
            {startSlot ? (
              <EditTimelockStart slot={startSlot} />
            ) : (
              <span>N/A</span>
            )}
            <nav className="divide-x items-center border rounded text-xs text-sky-700">
              <button onClick={() => setModal("start")} className="px-2 py-1">
                Change
              </button>
              <button
                onClick={() => setStartSlot(undefined)}
                className="px-2 py-1"
              >
                Remove
              </button>
            ...
            {modal === "start" && (
              <Modal
                className="bg-white p-4 rounded sm:w-full md:w-1/2 lg:w-1/3 space-y-1"
                onBackgroundClick={closeModal}
              >
                <h2 className="font-semibold">Start Slot</h2>
                <SlotInput
                  className="space-y-2"
                  confirm={confirmStartSlot}
                  cancel={closeModal}
          ...
          <div className="flex items-center space-x-2">
            ...
            {expirySlot ? (
              <EditTimelockExpiry slot={expirySlot} />
            ) : (
              <span>N/A</span>
            )}
            <nav className="divide-x items-center border rounded text-xs text-sky-700">
              <button onClick={() => setModal("expiry")} className="px-2 py-1">
                Change
              </button>
              <button
                onClick={() => setExpirySlot(undefined)}
                className="px-2 py-1"
              >
                Remove
              </button>
      ...
      <div>
        <header className="px-4 py-2 bg-gray-100">
          ...
          <p className="text-sm">
            {allRecipients.length > 0
              ? "The change caused by this transaction or all remaining assets in the treasury will be sent to this address (default to the treasury address). DO NOT MODIFY IT UNLESS YOU KNOW WHAT YOU ARE DOING!"
              : "All assets in this treasury will be sent to this address."}
          </p>
          {allRecipients.length > 0 && (
            <p>
              <label className="text-sm items-center space-x-1">
                <input
                  ...
                  onChange={() =>
                    setIsChangeSettingDisabled(!isChangeSettingDisabled)
                  }
                />
        ...
        <div className="p-4 space-y-2">
          ...
          {!willSpendAll && allRecipients.length > 0 && (
            <div className="space-y-1">
              <label className="flex block border rounded overflow-hidden ring-sky-500 focus-within:ring-1">
                <span className="p-2 bg-gray-100 border-r">
                  Least Change ADA
                </span>
                <ADAInput
              ...
              <div className="text-sm">
                Default to 5. The more tokens you have the larger it needs to
          ...
          {!isChangeSettingDisabled && allRecipients.length > 0 && (
            <div>
              <label className="items-center space-x-1">
                ...
                />
                <span>
                  Send all remaining assets in the treasury to this address
                </span>
              </label>
            </div>
          )}
      ...
      <footer className="flex p-4 bg-gray-100 items-center">
        <div className="grow">
          {txResult.isOk && (
            <p className="flex space-x-1">
              ...
              <span>
                <ADAAmount
                  lovelace={BigInt(txResult.data.body().fee().to_str())}
                />
              </span>
            </p>
          )}
          {!txResult.isOk && (
            <p className="flex space-x-1 text-red-500 items-center">
              <XCircleIcon className="h-4 w-4" />
              <span>
                {txResult.message === "The address is invalid."
        ...
        <nav className="flex justify-end space-x-2">
          ...
          <button
            ...
            onClick={() =>
              ...
              setStakeDeregistration(
                cardano.createDeregistrationCertificate(rewardAddress),
              )
            }
          >
            Deregister
          ...
          {modal === "delegation" && (
            <Modal
              className="bg-white p-4 rounded w-full lg:w-1/2"
              onBackgroundClick={closeModal}
            >
          ...
          {txResult.isOk && (
            <Link
              className="flex p-2 rounded bg-sky-700 text-white space-x-1 items-center"
              href={getTransactionPath(txResult.data)}
            >
              <span>Review</span>
      ...
      {stakePools && (
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          {stakePools.map((stakePool) => (
            <li key={stakePool.id}>
              <StakePoolInfo stakePool={stakePool} delegate={delegate} />
            </li>
          ))}
        </ul>
      )}
...
const fetchStakePoolMetaData = async (
  url: string,
): Promise<StakePoolMetaData> =>
  fetch(url)
    .then((response) => {
