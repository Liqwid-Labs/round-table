...
const removeTrailingZero = (value: string): string =>
  value.replace(/(\.[0-9]*[1-9]+)0*$/, "$1").replace(/\.0+$/, "");
...
const CurrencyInput: FC<{
  ...
  const changeHandle: ChangeEventHandler<HTMLInputElement> = useCallback(
    (event) => {
      const [i, f] = event.target.value.split(".", 2);
      const number = BigInt(
        i + (f || "0").slice(0, decimals).padEnd(decimals, "0"),
      );
      onChange(number);
    },
    [decimals, onChange],
...
const AssetAmount: FC<{
  ...
  return <span className={className}>{`${value} ${symbol}`}</span>;
};
...
const LabeledCurrencyInput: FC<{
  ...
  const { decimal, value, onChange, min, max, maxButton, symbol, placeholder } =
    props;
