...
const Panel: FC<{
  ...
  return (
    <div
      className={[
        "border-t-4 border-sky-700 bg-white rounded shadow overflow-hidden",
        className,
      ].join(" ")}
    >
      {children}
...
const CopyButton: FC<{
  ...
  return (
    <button
      className={className}
      disabled={disabled || isCopied || !content}
      onClick={click}
    >
...
const NavLink: FC<{
  ...
  const isOnPage = useMemo(() => {
    ...
    return (
      href === route ||
      parentPaths.every((name, index) => name === currentPaths[index])
    );
  }, [href, router.route]);
  ...
  return (
    <Link
      href={href}
      className={[className, isOnPage ? onPageClassName : ""].join(" ")}
    >
      {children}
    </Link>
  );
  ...
  return (
    <aside className="flex flex-col w-20 bg-sky-900 items-center text-white">
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        href="https://discord.gg/BGuhdBXQFU"
      >
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        href="https://github.com/ADAOcommunity/round-table"
      >
        <svg
    ...
    <div className="flex space-x-1 justify-between items-center p-4 bg-inherit">
      <div className="w-2/3">
        ...
        <div className="text-sm font-normal">
          {lovelace !== undefined ? (
            <ADAAmount lovelace={lovelace} />
          ) : (
            <SpinnerIcon className="animate-spin w-4" />
          )}
        </div>
      </div>
      <div>{children}</div>
...
const PersonalWalletListing: FC<{
  ...
  const isOnPage: boolean = useMemo(
    () => router.query.personalWalletId === wallet.id.toString(),
    [router.query.personalWalletId, wallet.id],
  );
  const addresses: string[] | undefined = useMemo(() => {
    if (!cardano) return;
    return Array.from(wallet.personalAccounts.values()).flatMap((account) =>
      cardano.getAddressesFromPersonalAccount(account, isMainnet(config)),
    );
  ...
  return (
    <WalletLink
      href={getPersonalWalletPath(wallet.id)}
      name={wallet.name}
      isOnPage={isOnPage}
...
const MultisigWalletListing: FC<{
  ...
  const isOnPage: boolean = useMemo(() => {
    ...
    if (typeof policyContent === "string") {
      ...
      const id = cardano
        ?.getPolicyAddress(policy, isMainnet(config))
        .to_bech32();
  ...
  return (
    <WalletLink
      href={getMultisigWalletPath(wallet.policy)}
      name={wallet.name}
      isOnPage={isOnPage}
      lovelace={lovelace}
    >
  ...
  const addresses: string[] = useMemo(() => {
    ...
    personalWallets?.forEach(({ personalAccounts }) => {
      personalAccounts.forEach((account) =>
        cardano
          .getAddressesFromPersonalAccount(account, isMainnet(config))
...
const Hero: FC<{
  ...
  return (
    <div
      className={[
        "rounded p-4 bg-sky-700 text-white shadow space-y-2",
        className,
      ].join(" ")}
    >
      {children}
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="w-full bg-sky-100 overflow-y-auto">
        {!isMainnet(config) && (
          <div className="p-1 bg-red-900 text-white text-center">
            You are using {config.network} network
          </div>
        )}
  ...
  return (
    <Portal id="modal-root">
      <div
        ...
        className="absolute bg-black bg-opacity-50 inset-0 flex justify-center items-center"
      >
...
const ConfirmModalButton: FC<{
  ...
  return (
    <>
      ...
      {modal && (
        <Modal
          className="bg-white p-4 rounded space-y-4 text-sm w-full md:w-1/3 lg:w-1/4"
        ...
        >
          ...
          <nav className="flex justify-end space-x-2">
            ...
            <button onClick={confirm} className={className} disabled={disabled}>
              {children}
            </button>
          </nav>
        </Modal>
      ...
      {modal && (
        <Modal className="w-80" onBackgroundClick={closeModal}>
          <div className="bg-white rounded overflow-hidden">
            <h2 className="bg-gray-100 p-2 text-center font-semibold">
              Open Remote Content
            </h2>
            <TextareaModalBox
              placeholder="Transaction URL/Hex or multisig wallet URL"
              onConfirm={confirm}
            >
...
const ConfigModalButton: FC<{
  ...
  const closeModal = useCallback(() => setModal(false), []);
  const openModal = useCallback(() => setModal(true), []);
  const switchAutoSync = useCallback(
          ...
          <div className="bg-white rounded overflow-hidden text-sm p-4 space-y-2">
            <div className="flex justify-between items-center">
              <div className="flex bg-sky-700 text-white divide-x border border-sky-700 rounded overflow-hidden">
                <button
                  className="p-1 disabled:bg-white disabled:text-sky-700"
                  onClick={() => setSubTab("basic")}
                  disabled={subTab === "basic"}
                >
                  Basic
                </button>
                <button
              ...
              <div className="space-y-2">
                ...
                {config.submitAPI && (
                  <div>
                    ...
                    <ul>
                      {config.submitAPI.map((api, index) => (
                        <li key={index}>{api}</li>
                      ))}
                    </ul>
                  </div>
            ...
            {subTab === "data" && (
              <div className="space-y-4">
                <div>
                  ...
                  <div>
                    User data has to be on the same network. For example, data
                    exported from testnet cannot be imported to mainnet.
                  </div>
                  <div className="text-red-500 font-semibold">
                    Data from V1 is not supported!
                  </div>
                </div>
                <div className="space-y-2">
                  <div>
            ...
            {subTab === "sync" && (
              <div className="space-y-2">
                <div>
                  ...
                  {config.gunPeers && (
                    <ul>
                      {config.gunPeers.map((peer, index) => (
...
const InlineEditInput: FC<{
  ...
  return (
    <>
      <span>{value}</span>
      <span>
        <button className="text-sky-700 p-1" onClick={editHandler}>
