...
const DownloadButton: FC<{
  ...
  return (
    <a href={URI} className={className} download={download}>
      {children}
    </a>
  );
};
...
const ExportUserDataButton: FC = () => {
  ...
  const keyHashIndices = useLiveQuery(async () => db.keyHashIndices.toArray());
  ...
  const filename = useMemo(
    () => `roundtable-backup.${config.network}.json`,
    [config.network],
  );
...
const ImportUserData: FC = () => {
  ...
  const click: MouseEventHandler<HTMLButtonElement> = useCallback(() => {
    ...
    if (userData.version !== "2") {
      notify(
        "error",
        "Incompatible version. Please recreate wallets and then migrate.",
      );
      return;
    }
    if (userData.version === "2") {
