...
const personalStakingKeyHash = (publicKey: Bip32PublicKey): Ed25519KeyHash =>
  publicKey.derive(STAKING_ROLE).derive(0).to_raw_key().hash();
const personalAccountPath = (accountIndex: number) =>
  PERSONAL_WALLET_PATH.concat(harden(accountIndex));
const multisigAccountPath = (accountIndex: number) =>
  MULTISIG_WALLET_PATH.concat(harden(accountIndex));
...
const toAddressString = (address: Address): string =>
  address.as_byron()?.to_base58() ?? address.to_bech32();

type Result<T> = { isOk: true; data: T } | { isOk: false; message: string };
...
class Cardano {
  ...
  public buildTxOutput(
    recipient: Recipient,
    protocolParams: ProtocolParams,
  ): SingleOutputBuilderResult {
    if (
      recipient.value.lovelace < this.getMinLovelace(recipient, protocolParams)
    ) {
  ...
  public getMinLovelace(
    recipient: Recipient,
    protocolParams: ProtocolParams,
  ): bigint {
    const { BigNum, TransactionOutput } = this.lib;
    if (!protocolParams.coinsPerUtxoByte)
      throw new Error("coinsPerUtxoByte is missing");
    const coinsPerUtxoByte = BigNum.from_str(
    ...
    const txOutput = TransactionOutput.new(
      address,
      this.buildCMLValue(recipient.value),
    );
    const minimum = this.lib.min_ada_required(txOutput, coinsPerUtxoByte);
  ...
  public buildCMLValue(value: Value): CMLValue {
    ...
    const cardanoValue = this.lib.Value.new(
    ...
    if (assets.size > 0) {
      ...
      assets.forEach((quantity, id, _) => {
        const policyId = ScriptHash.from_bytes(
          Buffer.from(getPolicyId(id), "hex"),
        );
  ...
  public createTxInputBuilder(input: TransactionOutput): SingleInputBuilder {
    ...
    if (input.tokens.length > 0) {
      ...
      input.tokens.forEach((token) => {
        ...
        const policyId = ScriptHash.from_bytes(
          Buffer.from(getPolicyId(assetId), "hex"),
        );
        const assetName = AssetName.new(
          Buffer.from(getAssetName(assetId), "hex"),
        );
    ...
    const txOuput = this.lib.TransactionOutput.new(
      this.parseAddress(input.address),
      value,
    );
  ...
  public getTxMessage(transaction: Transaction): string[] | undefined {
    ...
    return (
      metadatum &&
      Array.from(toIter(metadatum), (metadata) => metadata.as_text())
    );
  }
  ...
  public buildSignatureSetHex(
    vkeys: Array<Vkeywitness> | Vkeywitness | undefined,
  ): string | undefined {
    if (!vkeys) return;
  ...
  public parseAddress(address: string): Address {
    ...
    if (Address.is_valid_byron(address))
  ...
  public createTxBuilder(
    ...
    const exUnitPrices = ExUnitPrices.new(
      UnitInterval.new(
        toBigNum(priceMemFraction.numerator),
        toBigNum(priceMemFraction.denominator),
      ),
      UnitInterval.new(
        toBigNum(priceStepFraction.numerator),
        toBigNum(priceStepFraction.denominator),
      ),
    );
  ...
  public getNativeScriptFromPolicy(
    policy: Policy,
    getKeyHash: (address: Address) => Ed25519KeyHash,
  ): NativeScript {
    const {
      Address,
      BigNum,
    ...
    switch (policy.type) {
      case "TimelockStart":
        return NativeScript.new_timelock_start(
          TimelockStart.new(BigNum.from_str(policy.slot.toString())),
        );
      case "TimelockExpiry":
        return NativeScript.new_timelock_expiry(
          TimelockExpiry.new(BigNum.from_str(policy.slot.toString())),
        );
    }
    ...
    switch (policy.type) {
      case "All":
        return NativeScript.new_script_all(ScriptAll.new(nativeScripts));
      case "Any":
        return NativeScript.new_script_any(ScriptAny.new(nativeScripts));
      case "NofK":
        return NativeScript.new_script_n_of_k(
          ScriptNOfK.new(policy.number, nativeScripts),
        );
    }
  }
  ...
  public getPolicyRewardAddress(
    ...
    return RewardAddress.new(
      networkId,
      StakeCredential.from_scripthash(script.hash()),
    );
  }
  ...
  public getRequiredSignatures(script: NativeScript): number {
    ...
    switch (script.kind()) {
      case NativeScriptKind.ScriptAll:
        return totalNumber;
      case NativeScriptKind.ScriptAny:
        return 1;
      case NativeScriptKind.ScriptNOfK:
        ...
        return nofK.n();
      default:
        throw new Error(`Unsupported Script Type: ${script.kind()}`);
    }
  }

  public createRegistrationCertificate(
    rewardAddress: string,
  ): Certificate | undefined {
    const { Address, Certificate, StakeRegistration } = this.lib;
    const credential = Address.from_bech32(rewardAddress)
      .as_reward()
      ?.payment_cred();
    if (!credential) return;
    return Certificate.new_stake_registration(
      StakeRegistration.new(credential),
    );
  }

  public createDeregistrationCertificate(
    rewardAddress: string,
  ): Certificate | undefined {
    ...
    const credential = Address.from_bech32(rewardAddress)
      ...
      ?.payment_cred();
    if (!credential) return;
    return Certificate.new_stake_deregistration(
      StakeDeregistration.new(credential),
    );
  }

  public createDelegationCertificate(
    rewardAddress: string,
    poolId: string,
  ): Certificate | undefined {
    ...
    if (!credential) return;
    return Certificate.new_stake_delegation(
      StakeDelegation.new(credential, poolKeyHash),
    );
  }

  public createWithdrawalBuilder(
    rewardAddress: string,
    amount: bigint,
  ): SingleWithdrawalBuilder | undefined {
    const { Address, BigNum, SingleWithdrawalBuilder } = this.lib;
    const address = Address.from_bech32(rewardAddress).as_reward();
    if (!address) return;
    return SingleWithdrawalBuilder.new(
      address,
  ...
  public readRewardAddressFromPublicKey(
    bytes: Uint8Array,
    isMainnet: boolean,
  ): RewardAddress {
    ...
    const credential = StakeCredential.from_keyhash(
      personalStakingKeyHash(publicKey),
    );
    return RewardAddress.new(networkId, credential);
  }
  ...
  public async getRootKey(
    ...
    return decryptWithPassword(wallet.rootKey, password, wallet.id).then(
      (plaintext) =>
        this.lib.Bip32PrivateKey.from_bytes(new Uint8Array(plaintext)),
    );
  }

  public async signWithPersonalWallet(
    ...
    const requiredKeyHashes: Uint8Array[] = requiredKeyHashHexes.map((hex) =>
      Buffer.from(hex, "hex"),
    );

    const keyHashIndices = await db.keyHashIndices
      .where("hash")
      .anyOf(requiredKeyHashes)
      .and(({ walletId }) => walletId === wallet.id)
      .toArray();

    keyHashIndices.forEach(({ hash, derivationPath }) => {
      const signingKey = derivationPath
        .reduce((key, index) => key.derive(index), rootKey)
        .to_raw_key();
  ...
  public async generatePersonalAccount(
    wallet: PersonalWallet,
    password: string,
    accountIndex: number,
  ): Promise<KeyHashIndex[]> {
    const rootKey = await this.getRootKey(wallet, password);
    const accountPath = personalAccountPath(accountIndex);
    const publicKey = accountPath
      .reduce((key, index) => key.derive(index), rootKey)
      .to_public()
      .as_bytes();
    wallet.personalAccounts.set(accountIndex, {
      publicKey,
      paymentKeyHashes: [],
    });
  ...
  public async generateMultisigAccount(
    wallet: PersonalWallet,
    password: string,
    accountIndex: number,
  ): Promise<KeyHashIndex[]> {
  ...
  public generatePersonalAddress(
    wallet: PersonalWallet,
    accountIndex: number,
  ): KeyHashIndex[] {
    ...
    const paymentKeyHash = publicKey
      .derive(PAYMENT_ROLE)
      .derive(index)
      .to_raw_key()
      .hash();
    ...
    return [
      {
        hash: paymentKeyHash.to_bytes(),
        derivationPath: paymentPath,
        walletId: wallet.id,
      },
      {
        hash: stakingKeyHash.to_bytes(),
        derivationPath: stakingPath,
        walletId: wallet.id,
      },
    ];
  }

  public generateMultisigAddress(
    wallet: PersonalWallet,
    accountIndex: number,
  ): KeyHashIndex[] {
    ...
    const paymentKeyHash = publicKey
      ...
      .to_raw_key()
      .hash();
    const stakingKeyHash = publicKey
      .derive(STAKING_ROLE)
      .derive(index)
      .to_raw_key()
      .hash();
  ...
  public getAddressesFromPersonalAccount(
  ...
  public getAddressesFromMultisigAccount(
