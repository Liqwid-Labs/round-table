...
const estimateDateBySlot = (slot: number, network: Network): Date =>
  new Date((slot - shelleyStart(network) + networkOffset(network)) * 1000);
const estimateSlotByDate = (date: Date, network: Network): number =>
  Math.floor(date.getTime() / 1000) +
  shelleyStart(network) -
  networkOffset(network);
const slotSinceShelley = (slot: number, network: Network): number =>
  slot - shelleyStart(network);
...
const getEpochBySlot = (slot: number, network: Network) =>
  Math.floor(slotSinceShelley(slot, network) / slotLength(network)) +
  epochBeforeShelly(network);
const getSlotInEpochBySlot = (slot: number, network: Network) =>
  slotSinceShelley(slot, network) % slotLength(network);
...
const deriveKeyFromPassword = async (
  ...
  crypto.subtle
    .importKey("raw", Buffer.from(password, "utf-8"), "PBKDF2", false, [
      "deriveBits",
      "deriveKey",
    ])
    .then((material) =>
...
const SHA256Digest = async (data: ArrayBuffer): Promise<ArrayBuffer> =>
...
const parseDerivationPath = (path: string): number[] =>
  path
    ...
    .map((index) => {
      if (index.endsWith("'"))
        return harden(parseInt(index.substring(0, index.length - 1)));
      return parseInt(index);
    });

const formatDerivationPath = (indices: number[]): string =>
  ["m"]
    .concat(
      indices.map((index) => {
