...
test("sumValues", () => {
  ...
  const valueB: Value = {
    ...
    assets: new Map([["token1", BigInt(10)]]),
  };
  const valueC: Value = {
    lovelace: BigInt(50),
    assets: new Map([
...
describe("GraphQL API", () => {
  ...
  const talkbackServer = talkback({
    ...
    port: 8080,
    tapeNameGenerator: (tapeNumber) =>
      ["graphql", `query-${tapeNumber}`].join("/"),
  });
  ...
  test("useUTxOSummaryQuery", async () => {
    ...
    if (data) {
      ...
      expect(utxo1.txHash).toBe(
        "3ec64a8784bddc1b1849a349fe88c01918a58e4d32636420c17aafe156f16f9c",
      );
      ...
      expect(utxo3.txHash).toBe(
        "42e1b09014989a06633ca999c6a5bb20724af4773e725567d138cecca24fc800",
      );
  ...
  test("useSummaryQuery", async () => {
    ...
    const { result } = renderHook(
      () =>
        useSummaryQuery({ variables: { addresses: [address], rewardAddress } }),
      { wrapper },
    );
    ...
    if (data) {
      ...
      if (summary) {
        ...
        expect(summary.assetBalances[1]?.asset.assetId).toBe(
  ...
  test("usePaymentAddressesQuery", async () => {
    const address =
      "addr_test1xzuh59uc243wuhpkcnfdha3flvmx5guf8thkctv8l75u2zzap4eefgsu8h5selu2aaeu29vh96rf99wcp5f0x60ldx6s2ad79j";
    const { result } = renderHook(
      () => usePaymentAddressesQuery({ variables: { addresses: [address] } }),
      { wrapper },
    );
  ...
  test("useTransactionSummaryQuery", async () => {
    const txHash =
      "829c0c98a4037f214abe197276ef8b53be3e313b139e73a87f7a8d0ff70ff735";
    const { result } = renderHook(
      () => useTransactionSummaryQuery({ variables: { hashes: [txHash] } }),
      { wrapper },
    );
  ...
  test("useStakePoolsQuery", async () => {
    ...
    const { result } = renderHook(
      () =>
        useStakePoolsQuery({ variables: { id: poolId, limit: 1, offset: 0 } }),
      { wrapper },
    );
    ...
    if (data) {
      ...
      expect(stakePools[0].hash).toBe(
        "e931eea8a3e9344656f3f233e55c32ea5056303b5c313015871bc57f",
      );
    }
  });
});
